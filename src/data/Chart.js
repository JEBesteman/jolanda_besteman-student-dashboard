...
const Chart = () => {
    ...
    const getAveragesAssignments = () => {
      const averagesAssignmentList = assignmentList.map((assignment) => {
        const assignmentData = data.filter((item) => assignment === item.assignment);
        ...
        const valueDifficult = totalDifficultPerAssignment / assignmentData.length;
        const valueFun = totalFunPerAssignment / assignmentData.length;
    ...
    // // Add label
    const averageRatingAssigmentsWithLabels = averageRatingAssigments.map(avg => ({
        assignment: avg.assignment,
        difficultyRating: avg.difficultyRating,
        funRating: avg.funRating,
        label: `Opdracht ${
          avg.assignment
        }, difficultyRating: ${avg.difficultyRating}, enjoymentRating: ${avg.funRating}`
